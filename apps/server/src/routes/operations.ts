...
export function registerOperationRoutes(app: FastifyInstance, db: PrismaClient, engine: DdnsEngine, scheduler: Scheduler) {
  app.get("/api/health", async (_request, reply) => {
    try {
      ...
      return { status: "ok", database: "ok", scheduler: schedulerState, latestIp, timestamp: new Date().toISOString() };
    } catch {
      return reply.code(503).send({ status: "error", database: "error", timestamp: new Date().toISOString() });
    }
  });
  ...
  app.get("/api/dashboard", { preHandler: requireAuth }, async () => {
    ...
    return {
      currentIp: { ipv4: latestIp?.ipv4 ?? null, ipv6: latestIp?.ipv6 ?? null, detectedAt: latestIp?.finishedAt ?? null },
  ...
  app.post("/api/ip/detect", { preHandler: requireAuth }, async () => {
    ...
    for (const family of ["IPV4", "IPV6"] as const) {
      ...
      await db.ipDetectionResult.createMany({
        data: outcome.attempts.map((attempt) => ({
          runId: run.id, family, provider: attempt.provider, success: attempt.success,
          address: attempt.address ?? null, error: attempt.error ?? null, durationMs: attempt.durationMs,
        })),
      });
    }
    ...
    await db.ipDetectionRun.update({
      where: { id: run.id },
      data: { finishedAt: new Date(), ipv4: ipv4 ?? null, ipv6: ipv6 ?? null, success: Boolean(ipv4 || ipv6) },
    });
    return { id: run.id, ipv4: ipv4 ?? null, ipv6: ipv6 ?? null, outcomes };
  });

  const execute = (trigger: RunTrigger, force = false) => async (_request: unknown, reply: FastifyReply) => {
  ...
  app.post("/api/ddns/force", { preHandler: requireAuth }, async (request, reply) => {
    if ((request.body as { confirm?: unknown })?.confirm !== true) {
      return reply.code(400).send({ error: { code: "CONFIRMATION_REQUIRED", message: "Force update requires explicit confirmation" } });
    }
  ...
  app.get("/api/history/runs", { preHandler: requireAuth }, async (request) => {
    ...
    const [items, total] = await Promise.all([
      db.ddnsRun.findMany({
        where, include: { logs: true }, orderBy: { startedAt: "desc" },
  ...
  app.get("/api/history/logs", { preHandler: requireAuth }, async (request) => {
    ...
    const [items, total] = await Promise.all([
      db.ddnsUpdateLog.findMany({
        where, include: { run: { select: { trigger: true, status: true, startedAt: true } } },
