...
export function decideUpdate(current: string, detected: string, force: boolean, checkOnly: boolean) {
  ...
  async run(options: { trigger: RunTrigger; force?: boolean; recordId?: string }) {
    ...
    try {
      ...
      const records = await this.db.managedDnsRecord.findMany({
        where: { enabled: true, ...(options.recordId ? { id: options.recordId } : {}) },
        include: { account: true, zone: true },
      });
      await this.db.ddnsRun.update({ where: { id: run.id }, data: { recordsTotal: records.length } });
      ...
      await mapConcurrent(records, 3, async (record) => {
        ...
        if (!address) {
          ...
          await this.db.managedDnsRecord.update({
            where: { id: record.id },
            data: { health: "ERROR", lastCheckedAt: new Date(), lastError: "Public IP detection failed" },
          });
          return;
        }
      ...
      const status = failed === 0 ? "SUCCESS" : failed === records.length ? "FAILED" : "PARTIAL";
      ...
      await this.db.ddnsRun.update({
        where: { id: run.id },
        data: { status: "FAILED", finishedAt: new Date(), summary: error instanceof Error ? error.message : "Run failed" },
      });
      throw error;
    }
  }
