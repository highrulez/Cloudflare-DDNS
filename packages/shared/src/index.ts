...
export const cloudflareTtlSchema = z.union([
  z.literal(1),
  z.literal(60),
  z.literal(120),
  z.literal(300),
...
export const recordPatchSchema = z.object({
  ...
  ttl: cloudflareTtlSchema.optional(),
...
export const createDnsRecordSchema = z
  .object({
    ...
    hostname: z.string().trim().min(1).max(253),
    type: recordTypeSchema,
    ipSource: z.enum(['DETECTED_IPV4', 'DETECTED_IPV6', 'CUSTOM']),
    customIp: z.string().trim().max(45).optional(),
    proxied: z.boolean().default(false),
    ttl: cloudflareTtlSchema.default(1),
    ddnsEnabled: z.boolean().default(true)
  })
  .superRefine((value, context) => {
    if (value.type === 'A' && value.ipSource === 'DETECTED_IPV6') {
      context.addIssue({
        code: 'custom',
        path: ['ipSource'],
        message: 'A records require an IPv4 source'
      });
    }
    if (value.type === 'AAAA' && value.ipSource === 'DETECTED_IPV4') {
      context.addIssue({
        code: 'custom',
        path: ['ipSource'],
        message: 'AAAA records require an IPv6 source'
      });
    }
    if (value.ipSource === 'CUSTOM' && !value.customIp) {
      context.addIssue({
        code: 'custom',
        path: ['customIp'],
        message: 'A custom IP address is required'
      });
    }
  });

export const manageExistingRecordSchema = z.object({
  accountId: z.string().min(1),
  zoneId: z.string().min(1),
  cloudflareRecordId: z.string().min(1).max(64),
  ddnsEnabled: z.boolean().default(true)
});

export const manageExistingRecordsSchema = z.object({
  records: z.array(manageExistingRecordSchema).min(1).max(500)
});

export const deleteCloudflareRecordSchema = z.object({
...
export type ManageExistingRecordInput = z.infer<typeof manageExistingRecordSchema>;
