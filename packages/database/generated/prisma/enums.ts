
/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
* This file exports all enum related types from the schema.
*
* 🟢 You can import this file directly.
*/

export const RecordType = {
  A: 'A',
  AAAA: 'AAAA'
} as const

export type RecordType = (typeof RecordType)[keyof typeof RecordType]


export const RecordHealth = {
  UNKNOWN: 'UNKNOWN',
  HEALTHY: 'HEALTHY',
  DRIFTED: 'DRIFTED',
  ERROR: 'ERROR',
  DISABLED: 'DISABLED'
} as const

export type RecordHealth = (typeof RecordHealth)[keyof typeof RecordHealth]


export const IpFamily = {
  IPV4: 'IPV4',
  IPV6: 'IPV6'
} as const

export type IpFamily = (typeof IpFamily)[keyof typeof IpFamily]


export const RunTrigger = {
  SCHEDULED: 'SCHEDULED',
  MANUAL_CHECK: 'MANUAL_CHECK',
  MANUAL_UPDATE: 'MANUAL_UPDATE',
  FORCE: 'FORCE',
  SETUP: 'SETUP'
} as const

export type RunTrigger = (typeof RunTrigger)[keyof typeof RunTrigger]


export const RunStatus = {
  RUNNING: 'RUNNING',
  SUCCESS: 'SUCCESS',
  PARTIAL: 'PARTIAL',
  FAILED: 'FAILED',
  SKIPPED: 'SKIPPED'
} as const

export type RunStatus = (typeof RunStatus)[keyof typeof RunStatus]


export const UpdateAction = {
  CHECKED: 'CHECKED',
  SKIPPED: 'SKIPPED',
  UPDATED: 'UPDATED',
  CREATED: 'CREATED',
  FAILED: 'FAILED',
  DISABLED: 'DISABLED',
  STOPPED_MANAGING: 'STOPPED_MANAGING',
  DELETED: 'DELETED',
  NO_IP: 'NO_IP'
} as const

export type UpdateAction = (typeof UpdateAction)[keyof typeof UpdateAction]


export const UpdateResult = {
  SUCCESS: 'SUCCESS',
  ERROR: 'ERROR',
  UNCHANGED: 'UNCHANGED'
} as const

export type UpdateResult = (typeof UpdateResult)[keyof typeof UpdateResult]


export const AuthAuditEventType = {
  LOGIN_SUCCESS: 'LOGIN_SUCCESS',
  LOGIN_FAILED: 'LOGIN_FAILED',
  LOGIN_RATE_LIMITED: 'LOGIN_RATE_LIMITED',
  TURNSTILE_FAILED: 'TURNSTILE_FAILED',
  LOGOUT: 'LOGOUT',
  SESSION_EXPIRED: 'SESSION_EXPIRED',
  MFA_ENROLLMENT_STARTED: 'MFA_ENROLLMENT_STARTED',
  MFA_ENABLED: 'MFA_ENABLED',
  MFA_CHALLENGE_FAILED: 'MFA_CHALLENGE_FAILED',
  MFA_LOGIN_SUCCESS: 'MFA_LOGIN_SUCCESS',
  MFA_RECOVERY_CODE_USED: 'MFA_RECOVERY_CODE_USED',
  MFA_RECOVERY_CODES_REGENERATED: 'MFA_RECOVERY_CODES_REGENERATED',
  MFA_DISABLED: 'MFA_DISABLED',
  REAUTH_SUCCESS: 'REAUTH_SUCCESS',
  REAUTH_FAILED: 'REAUTH_FAILED',
  REAUTH_RATE_LIMITED: 'REAUTH_RATE_LIMITED',
  CLOUDFLARE_CREDENTIAL_ADDED: 'CLOUDFLARE_CREDENTIAL_ADDED',
  CLOUDFLARE_CREDENTIAL_CHANGED: 'CLOUDFLARE_CREDENTIAL_CHANGED',
  CLOUDFLARE_CONNECTION_REMOVED: 'CLOUDFLARE_CONNECTION_REMOVED',
  DNS_RECORD_DELETED: 'DNS_RECORD_DELETED',
  PASSWORD_CHANGED: 'PASSWORD_CHANGED',
  SECURITY_SETTING_CHANGED: 'SECURITY_SETTING_CHANGED'
} as const

export type AuthAuditEventType = (typeof AuthAuditEventType)[keyof typeof AuthAuditEventType]
